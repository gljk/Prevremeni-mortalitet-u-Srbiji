...
```
```{r causes_and_treemaps, message=FALSE, warning=FALSE}
...
# Serbia@data$pm25
# 
# Zagadjenje <- left_join(Serbia@data, 
# 
#  umrli %>% filter(Godina.materijala %in% c(2017:2019),
#                  Starost>30,
#                 Starost < YPLL_Age) %>%
#     mutate(YPLL = (YPLL_Age - Starost) * Broj.pojava) %>% group_by(Opstina) %>% summarise(YPLL=sum(YPLL)/3, Broj=sum(Broj.pojava)/3) %>% mutate(code=as.numeric(Opstina))) %>% mutate(YPLLpm2.5=(pm25/10*6.2*YPLL)/100, BrojPM25=(pm25/10*6.2*Broj)/100) 
# 
