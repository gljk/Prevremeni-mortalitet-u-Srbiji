...
pop %>% filter(god==2020, IDTer=="RS",IDPol!=0, IDStarost!=0) %>% mutate(Starost=c(0:85,0:85) , Pol=as.character(IDPol))) %>% 
  mutate(`Specifična stopa smrtnosti`=Smrti/vrednost*1000, Pol=ifelse(Pol=="1","Muški", "Ženski")) %>% ggplot() + geom_line(aes(x=Starost, y=`Specifična stopa smrtnosti`, col=Pol),size=1) + theme_tufte() + scale_color_manual(values = c("#377eb8", "#e41a1c"))+ scale_x_continuous(breaks=scales::extended_breaks(n=20)) +ylab("Specifična stopa smrtnosti (‰)")
...
pop %>% filter(god %in% c(2017,2018,2019) , IDTer=="RS",IDPol!=0, IDStarost!=0) %>% mutate(Starost=rep(c(0:85,0:85),3) , Pol=as.character(IDPol))) %>% group_by(Pol, Starost) %>% summarise(Smrti=sum(Smrti), vrednost=sum(vrednost)) %>% mutate(`Specifična stopa smrtnosti`=Smrti/vrednost*1000, Pol=ifelse(Pol=="1","Muški", "Ženski")) %>% ggplot() + geom_line(aes(x=Starost, y=`Specifična stopa smrtnosti`, col=Pol),size=1) + theme_tufte()+ scale_color_manual(values = c("#377eb8", "#e41a1c"))+ scale_x_continuous(breaks=scales::extended_breaks(n=20)) + ylab("Specifična stopa smrtnosti (‰)")
