...
#Računanje standardizovanog pokazatelja izgubljenih godina potencijalnog života umrlih
 stadnardize_ypll(umrli, pop, standardpop75, c( 2020),YPLL_Age )
...
#NoNCovid standardized YPLL map
  ...
  buildMap(
    map = Serbia,
    foritfied_map = SRMAP,
    motality_dataframe = standardized_YPLL_Avoidable_2017_2019,
    variable = "YPLLi",
    paleta = "OrRd"
  )
...
Serbia@data$wage <- left_join(Serbia@data,zaradaOpstina %>% filter(skrIndikator=="Просечне бруто зараде") %>% mutate(code=as.numeric(IDTer) ) %>%  select(code, Vrednost) %>%  group_by(code) %>% summarise(Vrednost=mean(as.numeric(Vrednost))))$Vrednost
