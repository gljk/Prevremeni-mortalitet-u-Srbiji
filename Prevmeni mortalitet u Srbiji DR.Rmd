...
standardized_YPLL_Covid <- left_join(
  left_join(
    ...
    mutate(YPLL = ifelse(is.na(YPLL), 0, YPLL)) %>% mutate(YPLLR = YPLL /
                                                             Pop * 100000),
  standardpop75
) %>% group_by(Opstina) %>% summarise( Median_AGE_YPLL=FindMedian(YPLL),
...
standardized_YPLL_TOTAL_2017_2019 <- left_join(
  left_join(
    pop %>% filter(god %in% c(2017,2018, 2019) , nPol == "Ukupno",!IDStarost %in% c("0", "V85")) %>% mutate(Starost =
                                                                                             as.integer(IDStarost)) %>% filter(Starost < 75) %>% group_by(Opstina = IDTer, Starost) %>% summarise(Pop =
                                                                                                                                                                                                    sum(vrednost)),
    umrli %>% filter(Godina.materijala %in% c(2017,2018, 2019) ,
                     Starost < YPLL_Age) %>%
      mutate(Broj.pojava = ifelse(is.na(Broj.pojava), 0, Broj.pojava)) %>%  mutate(YPLL = (YPLL_Age - Starost) * Broj.pojava) %>% group_by(Opstina, Starost) %>% summarise(YPLL = sum(YPLL))
  ) %>%
    mutate(YPLL = ifelse(is.na(YPLL), 0, YPLL)) %>% mutate(YPLLR = YPLL /
...
Serbia <- left_join(
  left_join(
    ...
    pop %>% filter(god %in% c(2017,2019, 2019) , nPol == "Ukupno",!IDStarost %in% c("0", "V85")) %>% mutate(Starost =
                                                                                             as.integer(IDStarost)) %>% filter(Starost < 75) %>% group_by(Opstina = IDTer,  Starost) %>% summarise(Pop =sum(vrednost))
   
  ) %>%
    mutate(YPLL = ifelse(is.na(YPLL), 0, YPLL)) %>% mutate(YPLLR = YPLL /
                                                             Pop * 100000) %>% group_by(Starost) %>% summarise(
  YPLL = sum(YPLL)/3,
  Pop = sum(Pop)/3
), 
standardpop75
)%>% group_by() %>%  summarize( YPLLi = sum((YPLL / Pop * stpop)*100000, na.rm = T),
  YPLLR = sum(YPLL) / sum(Pop) * 100000)
...
standardized_YPLL_TOTAL_2017_2019 <- left_join(
  ...
  standardpop75) %>% group_by(Opstina) %>% summarise( Median_AGE_YPLL=FindMedian(YPLL),
  YPLLi = sum(YPLLR * stpop, na.rm = T),
  YPLLR = sum(YPLL) / sum(Pop) * 100000,
  YPLL = sum(YPLL),
  Pop = sum(Pop)
...
```{r GIS i kartiranje, message=FALSE, warning=FALSE}
...
cortestdata<- left_join(Serbia@data %>% mutate(code=as.character(code)), standardized_YPLL_TOTAL_2017_2019, by=c("code"="Opstina"))
